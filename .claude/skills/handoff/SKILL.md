...
- **読み手規則**(共通・handoff は**議論文であって監査証跡ではない** — 監査証跡は system of record が担う):
  1. **結論先行**: ヘッダの次の 1 文が結論。「半分」「一部」「概ね」のような、次を読まないと指示対象が分からない抽象語で結論を代替しない —
     対象を数えて列挙する(例:「懸念は 2 つ。①…か ②…か。①は問題なし・②は一部機能」)。
  2. **語彙**: 読み手が本文だけで意味を取れない内部語(ツール名・検査 id・工程やラウンドの内輪名・略号)は、初出で普通の言葉に置き換えるか 1 句で言い換える。
     handoff で新しい造語(独自の指標名など)を導入しない — 普通の言葉で言えることは普通の言葉で言う(例:「探索収率」でなく「未知の問題をどれだけ見つけているか」)。
  3. **証拠の配置**: 本文は判断に必要な事実だけ。個別の番号・件数・例外・経緯の列挙は、パケットの後の `付録(根拠・返答不要)` か system of record へ。
     数値は「それが変われば結論が変わるもの」だけ本文に残す。
  4. **裁定の単一化**(DECIDE / DISCUSS): 人間に問うのは実際に裁定が要る点だけ。その裁定に付随する設計方針は「この裁定の前提」として本文に述べ、承認項目として並べない
     (番号を分けるのは独立に採否できる項目だけ — 契約の "independently decidable" の裏面)。
...
```text
[DISCUSS / PAUSED]                    ← 読み手規則の適用例: 結論先行・内部語なし・証拠は付録・問いは 1 点
...
実装規則の読み手規則(§2.2・P5〜P8・§3 の DISCUSS 例)= ECO-076(2026-09-14・第三者コメント「監査証跡としては良いが議論文としては悪い」への応答・契約は不変)。
