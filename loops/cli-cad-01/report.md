...
| **H2(翻訳層の位置予測)** | **支持** — divergence 8 クラス(≧3 で判定可)・**P1 該当 8/8 = 100%**(閾値 2/3)。P2(終了契約)・P3(環境前提)の divergence は 0 |
...
**結論**: 構成的適格条件を満たす CLI-CAD 候補は、転移実測(認定試験)に合格した — 別ベンダーの fresh 製造者が CAD のみから「CAD で規定した意味論」の製品を製造し、CAD のみから独立導出された oracle がそれを判定できた。CAD 適格性判定の媒体非依存化は CLI 媒体 N=1 で成立。
...
予測どおり divergence は P1(出力契約の暗黙次元)へ全数集中した。P2(終了契約)= 0 は、CAD が exit 写像を表形式で明示規定したため翻訳で落ちる余地がなかったことを意味する(翻訳層は「規定が薄い面」に生じる、という §10 の機序と整合)。fixture 較正で検出した f4 起票欠陥(タイトル部分文字列衝突)は、CAD→fixture 翻訳層の職人芸残留の実測でもある。
...
- OBS-20260711-09 への更新: codex プラグインが gpt-5.6-sol へ到達(transfer-04 時の 5.5 ゲートから変化 — 統合層の識別ゲート更新を実測)。
