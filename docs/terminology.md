...
### 表面
**External Surface / 知識依存部品。** ffmpeg コマンド文法・画面の画素など、要求から導けず外部仕様/設計トークンの知識転記を要し、受入に execution/golden + 人間判断を要する部品。観測した 12 件のずるのうち成果物上に発現した 4 件はいずれもここで出た(核 0。残りは oracle 5・process 1・bom-metamodel 2 — 盲検再分類 2026-07-11)。
