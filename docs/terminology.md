...
### Design System BOM
**Design System BOM / デザインシステム部品台帳(candidate)。** UI-CAD が要求する Card / CTA / Chip / Badge / IconButton などを、E-BOM surface 部品(E-DESIGN-*)と K-BOM design knowledge(K-DESIGN-*)の組として管理する補助台帳。正式な新BOM層ではなく、E-BOM/K-BOM/Control Plan へ同期するための UI surface カバレッジ表。
