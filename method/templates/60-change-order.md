...
- producer: <EQ-NNN — 製造担当の設備 ID を記述する欄(設備台帳を持つなら、その ID)。**製品リポではこの欄は記述欄であり、機械的な導出・検証・強制は存在しない**(機械化は BomDD 方法論リポの自己適用でのみ〔ECO-072〕・製品向けは運転層配布の第 2 弾候補〔EXP-20260914-01〕)>
- inspector: <EQ-NNN — 独立検査併用時の検査官設備 ID を記述する欄(同上: 製品リポでは記述欄・独立性の判定は人間の配員規律による)>
...
## 5. 回帰+変更受入(失敗5分類で帰属)
- **受入節の書式(二部形・ECO-080)**: 受入**条件**と受入**結果**を同じ文形で書かない。
  - 受入条件(製造前に凍結・結果を書かない): `- V<n>(条件): <満たすべき状態> であること — 検査法: <手順・grep・コマンド>`
  - 受入結果(受入時に記入・条件行は書き換えない): `- V<n>= PASS | FAIL | UNMEASURABLE(観測: <座標= ログ / commit / run id>)`。
    UNMEASURABLE は観測欄に**測定不能の原因**(計器の不能・権限不足・環境差・入力欠落のどれか)と試みたコマンドを書き、PASS に数えない。
  - 「全 PASS」「緑」「12/12」のような結果の文形は**観測行にだけ**書く。条件と結果が同形だと、条件を結果へ転記しても文面が変わらず、観測の有無が記録から読めない
    (由来: BomDD ECO-080 — 意味判定モデルによる設備認定で、結果文形の受入節 9/34 が「実施済み」と読まれた。人間の読者にも同じ)。
