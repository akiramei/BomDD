...
- 技術確認者(非知覚媒体〔CLI/API 等〕の案件がある場合は必須 — 未割当の案件は凍結不可):
- 環境準備責任者(途中参加者への作業環境の供給):

## 参加供給(provisioning — 途中参加者向け必須欄)
<!-- joining-a-project.md §0.5 の正本。空欄のまま参加者を迎えない —
     欠けている場合、参加者は開始・裁定・凍結を停止してよい(参加者の落ち度ではない)。 -->
- 対象リポジトリ(どれを・いくつ・どのブランチ/コミットで):
- コマンドの実行ディレクトリ:
- golden 実機の提供方法: 起動済み環境 / 再現可能な起動手順 / 操作を支援する技術担当者(いずれか必須)
- 作業環境(Claude Code 導入・認証済み)の供給手順または供給者:
