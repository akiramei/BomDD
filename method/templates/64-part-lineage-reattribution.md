...
| UI-BOM legacy `items[].ebomCandidate.items[]` | | | | |
| UI-BOM legacy `meta.ebomItemsReferenced[]` | | | | |
...
- 存在する artifact は PLM が参照フィールドを認識できており、coverage-gap / unrecognized-schema が 0 である。
