...
| GUI 案件で下記「Phase 1.5 完了条件」を満たさない | **Phase 1.5(UI/UX デザイン)** | モック作成は**自由創作**(HTML の動くモック推奨 — 作成は縛らず検査で縛る)→ `/bomdd-refmodel`(製品につき 1 回・skip 可=理由必須)→ `/bomdd-mock-lint`(矛盾・blocking はモックへ差し戻し)→ 解錠後 `/bomdd-ui-cad`(ui-ir-ui-bom.md §12.1 の**双子出力**= DOM snapshot+PNG) |
| `20-spec.md` がテンプレのまま | Phase 2 | phase2-spec.md。GUI なら CAD のモック+UI-BOM(Phase 1.5 の産物)が入力 |
...
| as-built に golden 承認あり | **納品済み** | 本スキルの役目終了。「以後の変更はすべて /eco-file が入口」と案内して停止 |
   ...
   **Phase 1.5 の完了条件(機械判定 — 「作った/検査した/CAD 化した気がする」は状態ではない)**:
   (a) GUI モックが存在する(`bomdd/ui/mock/` または CAD リポ)、(b) `ui-mock-inspect.py gate-cad`
   が解錠= blocking 0、(c) `ui-cad-gate.py` GU1–GU6 が PASS、(d) §12.1 の DOM snapshot+PNG
   双子出力が保存されている。判定は治具の exit code で行う。
