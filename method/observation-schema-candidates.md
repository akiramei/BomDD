...
7. **見出しの恒等式**(harness ECO-003 還元・2026-07-10)。見出し値が下位分類を網羅する総数として提示される場合、「分解して残す」とは、分類が相互排他的かつ網羅的であり、下位分類の和が見出し値と一致することまでを含む。分類外の独立カウンタが存在する場合は、見出しの恒等式へ明示的に含めるか、非網羅的な補助指標であることを宣言する。これを欠くと、未知の変更先が存在するにもかかわらず under 0 が成立し得る。初出: impact-retrospective 採点規約 v1→v2、Plm ECO-003 の unmapped-only。
