...
> **This document set is intentionally NOT a JSON Schema.** 候補表の段階で**意図的に止めている**。「schema 候補なのに schema が無い」のは欠落でなく設計判断——硬化(`schemas/draft/*.json`)は早すぎる形式化(片ドメイン構造の焼き込み)を避けるため、§5 の昇格条件を満たすまで保留する。
