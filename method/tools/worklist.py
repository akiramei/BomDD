...
#
# 検出力の限界(宣言 — ECO-047: /calibrate フル掃引〔第 5 掃引・2026-09-01〕からの確定):
#   (1) W1〜W6 が測るのは**マーカー行の形式整合のみ** — 意味整合(evidence 座標の実在・
#       source ECO の実在・回収判定の妥当性)は対象外設計。structured 判定も片側
#       (EXP-20260830-04 — 「新規項目ゼロの節」を弁別しない)。
#   (2) 縮退境界: ID 連番は 2 桁(\d{2})・watch の N は単桁 — 超過は**警告(W6/W4)つき
#       非計上へ縮退**する(沈黙はしないが一覧から落ち、診断文は桁上限という真因を
#       名指さない)。現運用(日付ごと採番)では遠い境界。
#   (3) fenced code block・blockquote・HTML コメント内は例示とみなし対象外(原則 3)—
#       誤ってそこへ書かれた**実記帳は警告なしに落ちる**(宣言済み境界)。
#   (4) 掃引の常設対照= --selftest(W1〜W6 全種+PROMOTION DUE+除外領域)。DUE の正腕は
#       実運用 2 件で接地済み(BomDD OBS-20260831-05/09 の昇格審査)。
