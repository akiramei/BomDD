...
#
# 採点規約(scale-01 で凍結・変更する場合は fork して宣言):
...
def main():
    ...
    if not units:
        sys.exit(f"manufacturing_units が {a.mbom} に見つからない")
    ...
    if changes is None:
        sys.exit("register の ECO リストキーが見つからない(--changes-key で指定)")

    field = a.impact_field
    if field is None:
        field = "impacted_bom" if any(c.get("impacted_bom") for c in changes) else "affected_refs"
    ...
    for c in changes:
        ...
        under, test_only, unmapped = [], 0, 0
        for f in sorted(actual):
            uid = attribute(f)
            if uid: actual_units[uid] += 1
            if uid in pred: continue
            if uid in a.test_unit: test_only += 1
            elif uid is None: unmapped += 1
            else: under.append((f, uid)); hub[uid] += 1
        over = sorted(u for u in pred if u not in actual_units)
        rows.append({"eco": eid, "commits": len(commits), "files": len(actual),
                     "pred_units": sorted(pred), "actual_units": sorted(actual_units),
                     "under": under, "test_only": test_only, "unmapped": unmapped, "over": over})

    under_ecos = [r["eco"] for r in rows if r["under"]]
    print(json.dumps({
        "field": field, "scored": rows, "skipped": dict(skipped),
        "summary": {
            "code_ecos": len(rows),
            "ecos_with_real_under": len(under_ecos),
            "real_under_files": sum(len(r["under"]) for r in rows),
