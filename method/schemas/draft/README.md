...
draft 改訂列: **ref-v0.1**(G3 ドライラン由来 4補正)→ **ref-v0.2**(初回製造由来 2補正)→ **ref-v0.3**(実リポ遡及由来 5補正・うち4件は「実物が正」)。卒業条件 3 の第1データ点。ref-v0.4〜v0.8 は ref-edges ヘッダの改訂台帳を参照。**ref-v0.9**(2026-07-16・harness ECO-013): ui-mock-extraction 族の**二方言両対応(同名異物の解消)** — 同一ファイル名に複数の成果物型が写像される場合、型は名前でなく**判別キー**(schemaVersion/artifactType 封筒の有無等)で判別し、宣言は全方言を被覆する(片方言のみの束縛は不一致側で検査が黙って消える — ref-v0.6 と同族)。uiId の domain 同期漏れ(ref-v0.3(c) 以来 13 日)も是正・派生同期は self-conformance C10(ECO-014)が恒久検査。
