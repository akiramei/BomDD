...
5. **検査の結論が、意味を保ったまま次の判断点へ届き消費される経路がある** — 非同期・遠隔で動く
   検査(CI 等)は、結果が生成されるだけでは足りない。**実行されていることは、観測されている
   ことを意味しない**(初出の形)。加えて、**観測されていることは、正しく伝わっていることを
   意味しない**(精密化・2026-07-27):

   検査には 4 段階ある — ①規則が存在する ②検査が実行される ③**結論が意味を保ったまま伝送される**
   ④結果が次の操作を制御する。破れは ③ でも起こり、その場合 ①②④ は健全に見える。
   ③④の実測で確認する項目:
   - **対象 revision との対応**(別 commit の結果を流用していないか)
   - **規則版との対応**(当時と異なる規則版の結果を根拠にしていないか・結果が古くないか)
   - **判定値が加工されていないか**(exit code をパイプ・`echo`・後続コマンドの成功で上書き
     していないか。CI が成功でも必須検査が実行されていない場合を含む)
   - **判断者が実際に消費したか**(取得手順があることと、判定前に読まれることは別)

   実測(単一反例による定義修正・harness ECO-024): ローカル検査は赤を出し、`exit=1` も
   `FAILED` も**正しく出力されていた**(①②③健全)。しかし検査と commit/push を同一の実行要求へ
   条件で結ばずに並べたため、**結果が判断者に消費される前に後続操作が実行**され、不適合のまま
   push された(**④の破れ**・CI が赤を出して事後検出)。破れは「値が壊れる」形とは限らず、
   **「正しい値が、間に合わない」形**でも起こる — 観測の順序が操作の順序より後ろにあれば、
   結線は存在しないのと同じである。「結果を確認する経路がある」という従来の書き方では、
   この破れは**条件を満たしているように読める** — 定義が不十分だった。
...
「設置済み」としない。**帰還方向は「結果が読めること」で満たされない** — 判定値が加工されず、
対象 revision・規則版との対応を保ったまま、判断者に消費されるまでが 1 本の経路である(上記 5)。
