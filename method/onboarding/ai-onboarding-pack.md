...
5. **検査の結論が、意味を保ったまま次の判断点へ届き消費される経路がある** — 非同期・遠隔で動く
   ...
   実測(単一反例による定義修正・harness ECO-024): ローカル検査は赤を出していた(①②健全)が、
   `検査 …; echo exit=$? && git commit …` と書いたため **`echo` の成功が chain を通し**、
   ③で判定値が失われて ④が誤動作し、不適合のまま push された(CI が赤を出して事後検出)。
   「結果を確認する経路がある」という従来の書き方では、この破れは**条件を満たしているように
   読める** — 定義が不十分だった。
