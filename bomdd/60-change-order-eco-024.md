...
1. **検査結果を見ずに commit/push した**: `self-conformance … ; echo exit=$? && git commit …` と
   書いたため、**echo の成功が chain を通してしまい**、C3 FAIL(diff 窓が開いている= head 未設定)の
   まま fix コミットが push された。是正= 次コミット(accept)で窓を閉じ、C3 PASS を実測。
   一般形= **判定の exit code をゲートに使うつもりの chain に、成功する表示コマンドを挟まない**
   (ゲート条件が echo にすり替わる。ECO-020「結果を観測する経路」の操作面版)。
