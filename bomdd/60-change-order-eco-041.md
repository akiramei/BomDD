...
- 対象 revision: `5bb71b9`(**ローカル HEAD と一致を確認**)
- run 識別子: 33421974126 — 結論: **PASS**(completed/success・headSha 照合済み)
- 観測日時 / 観測主体: 2026-09-01 / 本 ECO の担当設備

## 6. クローズ

- diff 監査の窓: baseline `3d55fcd` → head `5bb71b9`(**窓閉鎖**)。窓内は
  `method/bomdd-playbook-v1.md`+台帳系のみ — 影響なし予測が的中。
- 受入: V1(append のみ — 挿入 10/削除 0)/ V2(全検査 PASS)/ V3(CI 緑)/ V4(diff 窓)
  すべて成立。較正は散文規則につき機械対照なし(正直宣言)— 遵守は次回の独立検査報告・
  集計引用の実運用で観測する。
- gate 裁定の残条件の記録: ③clean known-good 特異度・同一対象再測定・他ベンダー比較=
  次回独立検査機会の測定候補 / ④battery 無変更を維持 / ⑤EXP 5 分類分離= 記帳済みと一致 /
  ⑥自己捕捉の主要観測格上げ= §0 に記録済み。
- このクローズが支持しないもの: 過去の検査記録・判定の再解釈(前向き適用)/ 検査官の
  精度一般(evidence class 併記でのみ語る)/ unknown 3 件の解消(次回測定候補)。
