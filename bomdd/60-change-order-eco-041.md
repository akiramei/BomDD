...
## 追記(2026-09-01): missed-trigger debt repayment — 事後較正 receipt

本 receipt は**事後適用**である — 受入節の執筆時(/calibrate trigger ①)に自発起動せず、
検出したのは機械でも receipt でもなく **2026-09-01 の人間の質問**(EXP-20260831-04
第 4 回観測)。**当時実施済みだったようには主張しない**。

- 査定した主張と判定(測定成立性×証拠資格):
  1. 「追補は append のみで既存文無変更」— **observed / 適格**(V1= diff 挿入 10 行/削除 0 行の
     機械実測)。
  2. 「本変更は既存検査を壊していない」— **observed / 適格**(V2 全 PASS+前後 commit の
     CI 緑系列の連続)。
  3. 「新様式(REJECT 意味論・報告様式・集計引用規則)は次回の独立検査で遵守される」—
     **unknown(理由コード: 未実行 — 前向き適用・次回検査機会待ち)**。資格判定なし。
- 検出した計器欠陥と帰属: なし。
- 検出力の限界: 事後査定につき「当時実施していれば防げた欠陥」の反実仮想は測れない —
  観測できたのは、結論が当時の受入と一致する(新規所見なし)ことまで。
