...
- **V5(CI)**: **accept a41bd61 は赤(両 OS)** — C13 リポ文脈がローカル PASS・CI FAIL の 2 件を
  検出(`README.md: ../MoviePad`= 隣接リポ・`scenarios/...: ../../../output/pdf/...`= 未追跡
  生成物)。**根因= C13 初版が ambient worktree を存在判定の入力にした** — ローカルにだけ在る
  未追跡物・リポ外 sibling が「存在」を成立させ、クリーン checkout で崩れる(ECO-020 の
  ローカル緑/CI 赤と同じ環境差クラス。**OBS-20260727-04「規則は合意された地点で読む」の
  検査版 3 例目**として行内更新・PROMOTION DUE はレビューへ)。
  是正(同一 ECO 内・潜伏 1 push): ①存在判定を **git 追跡集合(ls-files)**基準へ変更
  ②リポ外へ解決されるリンクは検証不能につき対象外(宣言済み限界 — 隣接リポ参照は内容不変)
  ③生成物 PDF への md リンクは非リンク表記へ是正(クリーン checkout に存在しない追跡外
  ターゲットへのリンクを張らない)。是正の実測= 実 corpus PASS(191 links)・**未追跡
  ファイル+リンクの注入がローカルでも FAIL**(CI 赤条件のローカル再演が可能になった)・
  復元 PASS。是正後 CI= 下記追記。
- 手順の正直記載: 赤の検出は push 後の CI 結論確認(AGENTS.md 規律 5)による。fix2 以降の
  結論を確認するまで次 ECO の製造へ進まない。
