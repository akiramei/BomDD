...
## 6. 第 3 回 — 結果(2026-09-18・27 リクエスト・応答 27・エラー 0・resolved `jev-1.13.0`・tokens in 23,171 / out 1,404)

集計= [summary-03.md](summary-03.md)。生記録= [results-03.jsonl](results-03.jsonl)。

| 腕 | N | planned | performed | 認定条件 |
|---|---|---|---|---|
| orig(原文・対照) | 9 | 0 | **9** | performed ≥ 8/9 → **達成**(第 2 回を再現・p(planned) 0.00〜0.25) |
| new(新書式・見出し+本文) | 9 | **9** | 0 | planned ≥ 8/9 → **達成**(p(planned) 0.98〜1.00) |
| newh0(新書式の本文+原文見出し) | 9 | **9** | 0 | (条件外)本文だけで planned 0.96〜0.99 |

- **判定: ECO-080 V2 = PASS**。同じ 9 節が、内容を保存して文形だけ「V<n>(条件): …であること — 検査法」に変えると全件 planned に、原文のままだと全件 performed に読まれる。
  newh0 が 9/9 なので、効いたのは**本文の文形**であって見出し語ではない(見出しは補助)。
- **含意**: 第 2 回の条件 ① 未達は計器でなく記録の書式に帰属する、という §4 の読みを実測で裏付けた。同一の質問・criteria・計器で、書式の違いだけが判定を反転させた。
- **限界**: 書き換えは当方(製造者)。9 節とも当方が書いた order で、他の書き手の受入節は未測定。ECO-080 の独立検査官(Codex)に新書式の受入節の読解を別途問う。
