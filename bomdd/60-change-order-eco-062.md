...
`Phase 5 継続(user DECIDE 2026-09-11 `1:C 2:A`: Phase 6 保留 → 検証器の是正 **ECO-066**〔理由コード・個体照合の既定化・tree 差分表示・測定不能の原因分離・範囲= 検証器のみ〕を先に閉じる → verified 後に運転員を変えた run-02〔同一治具・ブリーフ v2= P5-04/07 の手順側〕)。**現在地= Phase 6 第 1 弾 ECO-067 verified+実 cell 実測 済(2026-09-11・§10.7: 入口 → Codex read-only 起動 → 台帳、N=1・cell 側は sandbox の temp 不能で測定不能= 設備認定の属性)**。user 裁定 A(2026-09-11)= 還元と小是正を先に: **還元・織り込み(playbook §3 A/A'・§13 C・commit f3974c9)済・ECO-068 verified(実 cell で前後実測)**。user 裁定 B→C(2026-09-11)= **案 B= ECO-069 verified(templates に range・実行環境の欄)→ 中断(C)**。再開時の候補= Phase 7 の起票(DISCUSS から: cell の判定を receipt として回収する経路〔P6-02〕・設備属性〔sandbox・temp〕・複数 executor・裁定キュー・§1-5 設備認定参照)。付随: ECO-069 で入口が LEDGER_INCONSISTENT(§5 見出しの早書き)を commit 前に止めた= F0 検出の実運用 1 例目(OBS-20260910-02 の予防側)。Phase 5 の残る限界= N=2・人間 1 名・V3 未測定(§10.6)。run-01 の結果は §10(fail-open 0/7 だが R3・run-02 は運転員の判断依存= qualification blocker)。(ECO-055 status は 2026-09-11 user DECIDE A で verified に解消)。**再開 2026-09-12(user 指示「Phase 7 を DISCUSS から」)**: DISCUSS→AGREE で第 1 弾= 設備台帳の属性化+独立性判定の機械化(P6-02 は第 2 弾・裁定キューは保留・stop_type→配送先は ECO-067 で済)→ ECO-072 filed → user DECIDE「A」完全形 → **製造・V2 で Phase 7 出口(成立しない組合せ〔producer= executor= EQ-001〕を入口が STOP INDEPENDENCE_FAIL・異系統 EQ-002 は起動)を実測 1 例**→ Codex r1(境界探索・REJECT 3 是正)→ r2(是正確認+回帰・ACCEPT)→ **ECO-072 verified(2026-09-12)= Phase 7 第 1 弾 完了**。user DECIDE「A」= 第 2 弾 P6-02 → 経路 DISCUSS(②入口が cell の報告を束ねる)に AGREE → ECO-073 filed → user DECIDE「A」全部 → 製造 → r1(境界探索・REJECT 5)→ r2(REJECT 2)→ r3 ACCEPT → **ECO-073 verified(2026-09-12)= Phase 7 第 2 弾 完了(P6-02 解消・入口が cell の判定と hash を台帳へ束ね、受入判定を台帳から転記)**。user DECIDE「1:A 2:A」(2026-09-12)= 第 3 弾+EXP-20260912-01 評価(暫定: handoff 31・DECIDE 4・欠陥 4 類の再演 0)→ DISCUSS(witness gate・台帳から導出・range)に AGREE → ECO-074 filed → user DECIDE「A」1 層 → 製造 → r1(境界探索・REJECT 7)→ r2 ACCEPT → **ECO-074 verified(2026-09-12)= Phase 7 第 3 弾 完了(verified 昇格 commit を入口が inspection gate で止める・台帳から導出・range 弁別)**。user DECIDE「A」(2026-09-14)= **Phase 7 完了(§7 出口の再認証: 出口「独立検査として成立しない組合せを機械が弾いた実測 1 例」= ECO-072 V2〔producer= executor= EQ-001 → STOP INDEPENDENCE_FAIL〕で成立。加えて第 2 弾 ECO-073〔cell の判定と hash を台帳へ〕・第 3 弾 ECO-074〔昇格 commit を inspection gate で止める〕)**。残項目= 申告 gate の機械拒否・二層化(CI)・裁定キューは保留(必要が実測されてから)。§7 の Phase 0〜7 は完了。配布 ECO の DISCUSS(第 1 弾= handoff のみ・運転層は第 2 弾)に user AGREE+境界条件(配員欄は製品リポでは記述欄・機械強制なし・bomdd-job 非依存化と配員検査は第 2 弾候補として記名追跡)→ ECO-075 filed → user DECIDE「A」→ 製造 → r1(境界探索・ACCEPT 所見 0)→ r2(是正確認+回帰・ACCEPT)→ **ECO-075 verified(2026-09-14)= 配布 第 1 弾 完了(handoff 契約が製品リポへ配布可能・core 環境非依存・配員欄は製品では記述欄)**。次= user 裁定: 第 2 弾(運転層の製品配布・EXP-20260914-01)/ 中断。`
...
Phase 6 狭い自動起動入口 ─── 完了 2026-09-11(ECO-067 verified・実 cell N=1・ECO-068/069 で還元と小是正・user 裁定で Phase 7 入口)
        ▼
Phase 7 複数 executor・裁定キュー ─── 完了 2026-09-14(ECO-072 独立性判定・ECO-073 判定回収・ECO-074 昇格 gate。裁定キュー・申告 gate 拒否・二層化は保留)
        │
        ▼
(次) 配布 第 1 弾 ECO-075 ─── 完了 2026-09-14(handoff 契約の core/adapter 分割・product-profile 正本化)
        │
        ▼
(次) 配布 第 2 弾候補= 運転層の製品配布(EXP-20260914-01・記名追跡)◀━━ ★ 現在地= user 裁定待ち
```
...
| 5 外部運転員試験 | Phase 4 の記帳+§0.1 の運転員仕様 4 点(unknown)の裏取り | run 台帳(非正本)・裁定材料の提示記録・known-bad 対照腕の結果 | EXP-20260910-02 fail-open 0・EXP-20260910-03 伝言ゲーム率の基準線 | 済 2026-09-11(run-01 Codex・ECO-066・run-02 人間: fail-open 0/7 ×2・判断依存 0・基準線 AI 0/4 / 人間 座標なし 2/2・§10・§10.6・user 裁定 A で出口) |
| 6 狭い自動起動 | Phase 5 で fail-open 0 | 単一入口(`bomdd-run <job>` 相当・コマンド単位の承認は維持) | 自動起動 job で witness 再検証が機械的に効いた実測 | user(ECO-067 verified 2026-09-11: V3 無害 cell で初回成立・fail-open 2 件は検査官発見→是正・実製造セルは未実施。出口を満たしたと見なすかは user) |
| 7 複数 executor | Phase 6+設備認定台帳の属性化 | stop_type→配送先の機械定義(§0.5 の 5 種)・独立性判定の機械化(§1-5) | 独立検査として成立しない組合せを機械が弾いた実測 1 例 | 済 2026-09-14(user DECIDE A: ECO-072 V2 で出口成立・ECO-073/074 で判定回収と昇格 gate まで。裁定キューは保留) |
