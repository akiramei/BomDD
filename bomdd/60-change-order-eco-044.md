...
- 対象 revision: `3a383e0`(**ローカル HEAD と一致を確認**)
- run 識別子: 33478208325 — 結論: **PASS**(completed/success・headSha 照合済み)
- 観測日時 / 観測主体: 2026-09-01 / 本 ECO の担当設備

## 6. クローズ

- diff 監査の窓: baseline `5b92aec` → head `3a383e0`(**窓閉鎖**)。窓内は process-core tools
  2 ファイル+台帳系+記帳訂正(improvements — 裁定 1 の別コミット・本 ECO の変更対象外)のみ。
- 受入: V1(全 5 項に一次記録座標・新規限界なし)/ V2(挿入のみ 18 行・py_compile 両成立)/
  V3(全検査 PASS・C11 判定不変)/ V4(CI 緑)/ V5(窓)成立。較正 receipt は §4。
- このクローズが支持しないもの: ui-cad-gate・worklist・self-conformance 旧検査の限界宣言
  (フル掃引時に確定 — 裁定 3/5/6)/ 治具本体の能力査定(独立検査 4 巡+将来掃引の領分)。
